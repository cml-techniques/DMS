...
namespace CMLTechQ.DMS
{
    public partial class Project2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!String.IsNullOrEmpty(Request.QueryString["Auth3"]))
                {
                    ...
                    if (_type == "1")
                        content.Attributes.Add("src", "NavWebForm1?" + Request.QueryString.ToString());
                    else if (_type == "2")
                        content.Attributes.Add("src", "NavWebForm2?" + Request.QueryString.ToString());
                    else if (_type == "3")
                        content.Attributes.Add("src", "NavWebForm3?" + Request.QueryString.ToString());
                    else if (_type == "4")
                    {
                        content.Attributes.Add("src", "NavWebForm4?" + Request.QueryString.ToString());
                    }
                ...
                else
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                     ...
                     if (_prjid == "51")
                    {
                        defaultNav = (Convert.ToInt32(GetDefaultNav1(defaultMenu))).ToString();
                    }
                    else
                    ...
                    if (_type == "1")
                    {
                        content.Attributes.Add("src", "NavWebForm1?" + _auth + "&Auth3=" + defaultFolder + "&Auth5=" + defaultNav);
                       // content.Attributes.Add("src", "WebForm3?" + _auth + "&Auth3=" + defaultFolder);
                    }

                    else if (_type == "2")
                        content.Attributes.Add("src", "NavWebForm2?" + _auth + "&Auth3=" + defaultFolder + "&Auth5=" + defaultNav);
                    else if (_type == "3")
                        content.Attributes.Add("src", "WebForm5?" + _auth + "&Auth3=" + defaultFolder);
                    else if (_type == "4")
                        content.Attributes.Add("src", "WebForm6?" + _auth + "&Auth3=" + defaultFolder);
        ...
        private string GetDefaultNav1(string folder_id)
        {
            string defaultnav = "0";
            BLL_Dml _objbll = new BLL_Dml();
            _database _objdb = new _database();
            ...
            _objdb.DBName = "DBCML";
            _objcls.Folder_id = Convert.ToInt16(folder_id);
            DataTable dt = _objbll.Get_DafaultNavFolder1(_objcls, _objdb);
            if (dt.Rows.Count > 0) defaultnav = dt.Rows[0][0].ToString();
            return defaultnav;
        }
